    ...
    while (shouldContinue) {
        ...
        when (readLine()) {
            ...
            "Create" -> {
                ...
                println("Направление поезда: $direction")
                println()

                val tickets = Tickets().passengersCount
                println("Продано билетов: $tickets")
                println()

                Wagon().fillWagon(tickets).forEach{ println("Вагон №${it.num} вмещает ${it.capacity} пассажиров") }
                println()

                var finishDirection = Carriage(direction, Wagon().fillWagon(tickets).size)
                println(finishDirection.toString())
                println()
            }
